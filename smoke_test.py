...
# 예수금 보정 — 입금 기록 없이 BUY만 → 현금 음수. 실제 예수금에 맞춘 보정 후 현금 ≈ 실제
_rec_txns = [
    {"date": "2026-06-01", "time": "09:30", "ticker": "AAPL", "side": "BUY", "price": 100.0, "shares": 5},  # cash -500
]
_computed = jc.replay(_rec_txns)[0]
_real = 1000.0
_adj = _real - _computed
_side = jc.DEPOSIT if _adj > 0 else jc.WITHDRAW
_brow, _berr = app.build_manual_row("2026-05-31", "00:00", "", _side, abs(_adj), None, "USD")
_rec_after = _rec_txns + [app.df_to_txns(_pd.DataFrame([_brow]))[0]]
_cash_after = jc.replay(_rec_after)[0]
check("예수금 보정 후 현금 ≈ 실제(1000)", _berr == "" and abs(_cash_after - _real) < 1e-6,
      f"computed={_computed}, after={_cash_after}")
